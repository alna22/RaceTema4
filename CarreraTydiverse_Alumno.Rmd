...
date:  "2025-03-12"  #Pondría la fecha del día actual
params:
  lang: ES
...
filter(cran, country %in% c("ES", "PT"))
...
```{r}
cran2 <- arrange(cran2, country, desc(r_version), ip_id)
cran2
```
...
```{r}
cran3 <- mutate(cran3, size_gb = size_mb / 2^10)
cran3
```
...
```{r}
avg_bytes <- summarise(cran, avg_bytes = mean(size, na.rm = TRUE))
avg_bytes
```
# Agrupamientos
...
```{r}
summarise(by_package, avg_size = mean(size, na.rm = TRUE))
```
...
cran %>%group_by(country) %>%  summarise(total_traffic = sum(size, na.rm = TRUE)) %>%filter(total_traffic > quantile(total_traffic, probs = 0.90)) %>%  arrange(desc(total_traffic)) %>% ggplot(aes(x = reorder(country, -total_traffic), y = total_traffic)) +
  geom_point() +  labs(x = "País", y = "Tráfico Total (Bytes)", title = "Países con Mayor Tráfico de Descargas") +
...
```{r}
library(readr)  
code_paises <- read_csv("GeoLite2-Country-Locations-es.csv", locale = locale(encoding = "UTF-8"))
glimpse(code_paises)

cran <- cran %>%
  filter(!is.na(country) & country != "")  
code_paises <- code_paises %>%
  filter(!is.na(country_iso_code) & country_iso_code != "")  

cran_continentes <- cran %>% inner_join(select(code_paises, country_iso_code, continent_name), by = c("country" = "country_iso_code"))

tráfico_continentes <- cran_continentes %>%group_by(continent_name)%>%summarise(total_TB = sum(size, na.rm = TRUE) / 2^40) %>%arrange(desc(total_TB)) 

library(ggplot2)

tráfico_continentes %>%ggplot(aes(x = reorder(continent_name, -total_TB), y = total_TB)) +geom_bar(stat = "identity", fill = "steelblue") + labs(x = "Continente", y = "Tráfico Total (TB)", title = "Tráfico de Descargas por Continente") +theme_minimal() +theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
students2_tidy <- students2 %>%pivot_longer(cols = -grade, names_to = c("sex", "class"),names_pattern = "(male|female)_(\\d)",values_to = "count")  

print(students2_tidy)
...
pivot_longer(cols = -name,names_to = c("class", "exam"),names_sep = "_", values_drop_na = TRUE)

print(students3_tidy)
...
students <- bind_rows(failed, passed)
...
sat_tidy <- sat %>%pivot_longer(cols = -score_range, names_to = c("part", "sex"),  names_sep = "_", values_to = "count")
...
ggplot(sat_tidy, aes(x = score_range, y = count, fill = sex)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ part) +labs(x = "Franja de Calificación", y = "Número de Estudiantes", title = "Diferencias entre Hombres y Mujeres en el SAT") + theme_minimal() +
...
year_value <- year(this_day)
month_value <- month(this_day)
day_value <- day(this_day)
weekday_value <- wday(this_day, label = TRUE, abbr = FALSE)
...
hour_value <- hour(this_moment)
minute_value <- minute(this_moment)
second_value <- second(this_moment)

print(paste("Hora:", hour_value))
print(paste("Minuto:", minute_value))
print(paste("Segundo:", second_value))
...
date1 <- ymd("1989 May 17")
date1

date2 <- mdy("March 12, 1975")
date2
...
salida <- update(salida, hours = 17, minutes = 34, seconds = 0)
salida
...
intervalo <- interval(ultima_vez, llegada_hk)

tiempo_transcurrido <- as.period(intervalo)
